...
public interface UserFacade {
     ...
     * 注册用户
     */
    UserResponse<Boolean> register(UserRegisterRequest request);

    /**
     * 用户登录
     */
    UserResponse<UserInfo> login(UserLoginRequest request);

    /**
     * 分页查询用户
     */
    UserResponse<PageResult<UserInfo>> pageUser(UserPageQueryRequest request);

    /**
     * 修改用户状态
     */
    UserResponse<Boolean> updateStatus(Long id, UserState status);
}
